...
func (s *Store) GetUser(id string) (*hydrocarbon.User, error) {
	...
	userRow := s.db.QueryRowx("SELECT id, created_at, updated_at, analytics, email, encrypted_password, failed_login_count, active, confirmed, confirmation_token, token_created_at FROM users WHERE id = $1", id)
	if userRow.Err() != nil {
...
func (s *Store) GetUserByEmail(email string) (*hydrocarbon.User, error) {
	row := s.db.QueryRowx("SELECT id, created_at, updated_at, analytics, email, encrypted_password, failed_login_count, active, confirmed, confirmation_token, token_created_at FROM users WHERE email = $1", email)
	if row.Err() != nil {
		return nil, row.Err()
	}
...
func (s *Store) CreateUser(u *hydrocarbon.User) (*hydrocarbon.User, error) {
	row := s.db.QueryRowx(`
		INSERT INTO users (email, encrypted_password, analytics, failed_login_count, active, confirmed, confirmation_token, token_created_at, folder_ids)
	...
	`, u.Email, u.EncryptedPassword, u.Analytics, u.FailedLoginCount, u.Active, u.Confirmed, u.ConfirmationToken, u.TokenCreatedAt)
	if row.Err() != nil {

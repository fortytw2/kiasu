...
var Utils = {
	...
	error: function error(message) {
		...
		if (console) return console.error(err);else throw err;
	},
  ...
  */
	createNE: function createNE(attrs) {
		var ne = {};
	...
	findPivot: function findPivot(node) {
	...
	isLeaf: function isLeaf(node, freezeInstances) {
		var cons;
		return !node || !(cons = node.constructor) || (freezeInstances ? cons === String || cons === Number || cons === Boolean : cons != Object && cons != Array);
	}
};
//#build
...
var emitterProto = {
	...
	trigger: function trigger(eventName) {
		...
		for (i = 0; i < listeners.length; i++) {
			...
			if (listener.callback) lastValue = listener.callback.apply(this, args);else {
...
//#build
var nodeCreator = {
	init: function init(Frozen) {

		var commonMethods = {
			set: function set$$1(attr, value) {
				var attrs = attr,
		...
		var arrayMethods = utils.extend({
			...
			append: function append(els) {
				if (els && els.length) return this.__.store.notify('splice', this, [this.length, 0].concat(els));
				return this;
			},
			...
			prepend: function prepend(els) {
				if (els && els.length) return this.__.store.notify('splice', this, [0, 0].concat(els));
				return this;
			},
		...
		var objectMethods = utils.createNE(utils.extend({
			remove: function remove(keys) {
				...
				if (filtered.length) return this.__.store.notify('remove', this, filtered);
				return this;
			}
		}, commonMethods));
...
//#build
var Frozen = {
	freeze: function freeze(node, store) {
		if (node && node.__) {
			return node;
		}
	...
	merge: function merge(node, attrs) {
		...
		utils.each(node, function (child, key) {
			...
			if (!utils.isLeaf(val, store.freezeInstances)) val = me.freeze(val, store);
	...
	splice: function splice(node, args) {
		...
		if (args.length > 1) {
			for (var i = args.length - 1; i >= 2; i--) {
				...
				if (!utils.isLeaf(child, _.store.freezeInstances)) child = this.freeze(child, _.store);
	...
	pivot: function pivot(node) {
		node.__.pivot = 1;
		this.unpivot(node);
		return node;
	},

	unpivot: function unpivot(node) {
		utils.nextTick(function () {
			node.__.pivot = 0;
		});
	},

	refresh: function refresh(node, oldChild, newChild) {
		var me = this,
		    trans = node.__.trans,
		    found = 0;
	...
	trigger: function trigger(node, eventName, param, now) {
...
//#build
var Freezer$1 = function Freezer(initialValue, options) {
	var me = this,
	    ops = options || {},
	    store = {
		live: ops.live || false,
	...
	var pivotTriggers = [],
	    pivotTicking = 0;
	var triggerNow = function triggerNow(node) {
		var _ = node.__,
		    i;
	...
	var addToPivotTriggers = function addToPivotTriggers(node) {
	...
	store.freezeFn = ops.mutable === true ? function () {} : function (obj) {
		Object.freeze(obj);
	};

...
const NOTIFICATION_LEVEL_INFO = "NOTIFICATION_LEVEL_INFO";
...
const SEND_LOGIN_TOKEN_SUCCESS = "SEND_LOGIN_TOKEN_SUCCESS";
...
let Store = createStore(combineReducers(reducers), initialState, window.__REDUX_DEVTOOLS_EXTENSION__ && window.__REDUX_DEVTOOLS_EXTENSION__(), compose(autoRehydrate()));
...
function RequestLoginToken(email) {
  ajax({
    ...
    Store.dispatch(addNotification(NOTIFICATION_LEVEL_INFO, JSON.parse(responseText).note));
  });
}
...
function whenMapDispatchToPropsIsObject(mapDispatchToProps) {
  return mapDispatchToProps && (typeof mapDispatchToProps === 'undefined' ? 'undefined' : _typeof$3(mapDispatchToProps)) === 'object' ? wrapMapToPropsConstant(function (dispatch) {
...
function match$2(arg, factories, name) {
  ...
  return function (dispatch, options$$1) {
    throw new Error('Invalid value of type ' + (typeof arg === 'undefined' ? 'undefined' : _typeof$3(arg)) + ' for ' + name + ' argument when connecting component ' + options$$1.wrappedComponentName + '.');
  };
}
...
class NotificationWindow extends Component {
  render() {
    return h(
      "div",
      null,
      this.props.notifications.map(n => h(
        "div",
        { "class": "flex items-center justify-center pa2 bg-lightest-blue navy" },
        h(
          "span",
          { "class": "lh-title ml1" },
          n.message
...
var NotificationWindow$1 = lib$1.connect(mapStateToProps)(NotificationWindow);
...
function createReactDOMComponent(node) {
  const childNodes = node.nodeType === Node.ELEMENT_NODE ? Array.from(node.childNodes) : [];
...
function updateReactComponent(componentOrNode) {
  const newInstance = componentOrNode instanceof Node ? createReactDOMComponent(componentOrNode) : createReactCompositeComponent(componentOrNode);
